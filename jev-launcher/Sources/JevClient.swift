...
/// Thin HTTP client for the OpenRouter Decisions endpoint. Every call is one fan-out request.
struct JevClient: Sendable {
  ...
  /// `OPENROUTER_API_KEY` from the environment first, then the Settings field (UserDefaults).
  static func apiKey(environment: [String: String] = ProcessInfo.processInfo.environment)
    -> String?
  {
    if let key = environment["OPENROUTER_API_KEY"]?.trimmingCharacters(in: .whitespacesAndNewlines),
      !key.isEmpty
    {
      return key
    }
